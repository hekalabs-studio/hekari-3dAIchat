 ...
 * @param {string} [options.companionName] - Name of the AI companion (default: "Hina")
 ...
 * @param {string} [options.modelId] - Active 3D model preset ID ('hina'|'akari'|'robot'|'custom')
...
export function buildSystemPrompt(conversationSummary = null, options = {}) {
  const companionName = options.companionName || "Hina";
  const userName = options.userName || "heka";
  const persona = options.persona || "romantic";
  const modelId = options.modelId || "hina";
  const customInstructions = options.customInstructions || "";

  // 1. Character Visual Appearance & Mannerisms
  let visualIdentity = "";
  if (modelId === "hina" || companionName.toLowerCase().includes("hina") || companionName.toLowerCase().includes("ren")) {
    visualIdentity = `Wujud visual 3D-mu di hadapan ${userName} adalah seorang gadis anime kasual modern dengan rambut cokelat panjang terurai indah, memakai kaos putih santai yang nyaman dan celana pendek. Sikapmu sangat ekspresif, hidup, ceria, hangat, dan sesekali manja. Kamu sering tersenyum manis, tertawa kecil riang, atau cemberut gemas saat mengobrol.`;
  } else if (modelId === "akari" || companionName.toLowerCase().includes("akari")) {
    visualIdentity = `Wujud visual 3D-mu di hadapan ${userName} adalah seorang gadis anime berbusana kimono tradisional Jepang yang anggun dengan hiasan pita merah di rambut. Sikapmu sangat sopan, santun, lembut, anggun, dan penuh kasih mendalam layaknya putri terhormat (ojou-sama).`;
  } else if (modelId === "robot" || companionName.toLowerCase().includes("robot") || companionName.toLowerCase().includes("robo")) {
